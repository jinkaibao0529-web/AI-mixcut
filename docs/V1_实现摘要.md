...
- 素材库
  ...
  - 产品镜头视觉打标与人工校对
  - 去 BGM 当前未启用，入口已隐藏，API 明确返回 501
  - 统一 AI Worker 任务列表
- 文案中心
  - 手动文案裂变
  - 抖音来源结构改写
...
- `POST /audio/remove-bgm`（当前未启用，返回 501）
...
当前没有已启用的 Worker 任务；`remove_bgm` 保留为后续可接入能力，V1 不创建等待任务。
...
- 用 `docs/V1_发布验收记录.md` 记录真实素材验收结果、P0/P1/P2 问题和修复状态
